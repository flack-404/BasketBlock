...
const PaymentTypes = () => {
  ...
  // Contract Addresses
  const BasketBlock_ADDRESS = '0x1773c3A8cEbB984E2B84660AE95e512C992fffBC';
  const BETTING_POOL_ADDRESS = '0xeBFb57D3b512c34B1a4D96737E20B32b70749b65';
  ...
  const fetchTokenBalance = async () => {
    ...
    try {
      ...
      const BasketBlockContract = new ethers.Contract(
        BasketBlock_ADDRESS,
        [
      ...
      const balance = await BasketBlockContract.balanceOf(address);
      const decimals = await BasketBlockContract.decimals();
  ...
  const handleDeposit = async (amount: string) => {
    ...
    try {
      ...
      const approveTx = await BasketBlockContract.approve(
        BETTING_POOL_ADDRESS,
        amountWei
      );
            ...
            <div className='space-y-6'>
              ...
              <div className='grid grid-cols-2 items-center justify-between gap-2'>
                ...
                <Button
                  ...
                  className='w-full px-5 py-3 text-white'
                  onClick={() =>
                    handleCopy(`${Number(tokenBalance).toFixed(2)} $FTO`)
                  }
                  disabled={loading}
                >
                  <span className='mx-auto w-full'>
                    {loading
                      ? 'Loading...'
                      : `${Number(tokenBalance).toFixed(2)} $FTO`}
                  </span>
                </Button>
              </div>
              <h2 className='!h2'>
                Bet Locked in BettingPool:{' '}
                {loading
                  ? 'Loading...'
                  : `${Number(totalInvested).toFixed(2)} $FTO`}
              </h2>
              <Button
                variant='outline'
                size='lg'
